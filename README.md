...
- ライセンスに[zlib](https://ja.wikipedia.org/wiki/Zlib_License)風ライセンスを採用。当ライブラリ利用時にライセンス条文等を表示したりする必要はない


# オンラインデモ

- 動作確認デモページ http://vnctst.tir.jp/vnctst-audio3-demo/
    - BGM変更のフェードアウトおよびフェードアウト中から同じ曲に戻る際のフェードイン対応の辺りが目玉です。雑に扱っても、この辺りを適切に対応してくれます
...
- 0.1.1 (2016-07-02)
    ...
    - `all-bgm-keys` `all-bgs-keys` `all-me-keys` `all-se-keys` は、
      ...
      (これにより `(:se/hoge vnctst.audio3/all-bgm-keys)`
