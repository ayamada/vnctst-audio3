...
音響ファイルを再生するライブラリは既存のものが多数あるものの、「ゲーム向け」としてはどれも後述の機能が足りていなかった。なので、この`vnctst-audio3`を作った。

「ゲーム向け」として、自分が必要としている要件は、具体的には以下の通り。

- SE系の管理とBGM系の管理が分かれている事
    - それぞれで求められる要件がかなり違う為。それぞれに求められる要件は以下の通り。
    - SE系
        - 同一SEの多重再生への対応
            - 違うSEとの多重再生には対応していても、同一SEの多重再生に対応していないライブラリが意外と多い
            - ゲームでは弾丸の射出音など、同一のSEが重なって再生される事になるケースが多い
    - BGM系
        - 曲変更に伴い自動的かつ適切に行われるフェードアウト/フェードイン処理
            - フェード自体をサポートしていないライブラリが多く、またフェードをサポートしていても扱いが不完全だったりするものしかない
                - 次の曲を再生する時に、現在の曲をフェードアウトしてから再生するようにキューイングしてほしい。このフェードアウトの途中でのキャンセル処理/更に違う曲への差し替えもサポートしてほしい
...
- 憶える量の少ない、必要最小限のインターフェース
    - 前述の「複雑な状態遷移」を内部に隠しつつ、基本的なインターフェース自体は非常にシンプルな「再生」「停止」関連のみに絞った
...
- RPGツクールを参考とした、 BGM, BGS, ME, SE 別の再生分類
    ...
    - MEは、ループ再生しない音源。BGMとは排他に再生可能(というか内部ではBGMの一種として扱われている)。ループしないBGMや、ジングル等に使う。
...
3. `(audio3/init!)` を実行しておく
    - 以下のキーワード引数を与える事により、挙動をカスタマイズする事が可能(しかし全省略でも問題ない)
        ...
        -  `:always-mute-at-mobile? 真偽値` - 真値を指定する事で、`User-Agent`ヘッダがモバイル環境っぽい場合に音響再生を常時ミュート状態にします。現時点ではモバイル対応に不完全な部分があり、もし不完全な対応を許容できない場合は真値を指定してください。
...
そこで、以下のような指定方法を可能とした。以後、この指定方法を「プリセット指定」と呼ぶ。
...
- BGM系のプリロードを行う
    ...
    - BGM/BGS/MEはどれも「BGM系」として、プリロード時には全て同じ扱いとなる
    - 注意点として、BGM系はSEとは違い、単に `playBgm()` 等を実行しただけでは内部キャッシュ状態とはならない。なぜならBGM系は「即座に再生しないといけない」という要件があるケースがあまりなく、また一曲のメモリ消費が大きい場合が多いからである。明示的に `preload-bgm!` を実行した時のみ、プリロード状態になる。
...
- 音が出ない環境の事を常に考慮する必要がある
    - これは「何らかの応答を返すのに音だけの表現にはしない(必ずビジュアルでのエフェクトも付ける)」という事と、「再生が一瞬で完了する(ので、音源の再生完了をトリガーとして何かを行うような事は避けた方がよい)」という事の二つの点に注意すればとりあえずok

- 一応、「oggとmp3」もしくは「oggとm4a」で、基本的なブラウザ環境(PCおよびモバイル)での再生はカバーできる、という事になっている
    - 「ogg」「mp3」「m4a」のいずれも一つだけだと、再生できない環境が出てしまう。よって「oggとmp3」「oggとm4a」のどちらかのセットを選ぶ事になる

- 違うドメインにあるファイルを再生する場合はCORS回りの設定が必要

TODO: もっとあった筈なので、あとで追記する
...
- 0.1.0-rc2 (2016-XX-XX)
    - js用モジュールの関数名を`snake_case`から`camelCase`に変更
    - ドキュメントの修正と追加
